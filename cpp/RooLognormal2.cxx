 /***************************************************************************** 
  * Project: RooFit                                                           * 
  * @(#)root/roofit:$Id: RooLognormal.cxx 44982 2012-07-10 08:36:13Z moneta $ *
  *                                                                           * 
...
ClassImp(RooLognormal2)


//_____________________________________________________________________________
RooLognormal2::RooLognormal2(const char *name, const char *title,
...
//_____________________________________________________________________________
RooLognormal2::RooLognormal2(const RooLognormal2& other, const char* name) : 
...
//_____________________________________________________________________________
Double_t RooLognormal2::evaluate() const
{
  ...
  std::cout << "evaluating " << this->GetName() << " at x=" << xv << ", k=" << k << ", m0=" << m0 << " (" << ret << ")" << std::endl;
  return ret ;
}
...
//_____________________________________________________________________________
Int_t RooLognormal2::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* /*rangeName*/) const 
{
...
//_____________________________________________________________________________
Double_t RooLognormal2::analyticalIntegral(Int_t code, const char* rangeName) const 
{
  assert(code==1) ;
  ...
  std::cout << "piece 1: " << RooMath::erf( TMath::Log(x.max(rangeName)/m0)/(root2*ln_k) ) << std::endl;
  std::cout << "piece 2: " << RooMath::erf( TMath::Log(x.min(rangeName)/m0)/(root2*ln_k) ) << std::endl;
  std::cout << "min: " << x.min(rangeName) << std::endl;
  std::cout << this->GetName() << " integral=" << ret << std::endl << std::endl;
  return ret ;
}
...
//_____________________________________________________________________________
Int_t RooLognormal2::getGenerator(const RooArgSet& directVars, RooArgSet &generateVars, Bool_t /*staticInitOK*/) const
{
...
//_____________________________________________________________________________
void RooLognormal2::generateEvent(Int_t code)
{
  assert(code==1) ;
