...
def legSpec(goptions, errorBand = "") :
    out = ""
...
class validationPlotter(object) :
    ...
    def validationPlot(self, note = "", fileName = "", legend0 = (0.3, 0.6), legend1 = (0.85, 0.85), reverseLegend = False, minimum = 0.0, maximum = None,
        ...
        stuff = []
        leg = r.TLegend(legend0[0], legend0[1], legend1[0], legend1[1], "" if (otherVars and obsKey) else "")
        leg.SetBorderSize(0)
        leg.SetFillStyle(0)
    ...
    def ratioPlot(self, note = "", fileName = "", legend0 = (0.3, 0.6), legend1 = (0.85, 0.88), specs = [], yLabel = "",
    	...
    	for spec in specs :
            ...
            if inDict(spec, "legend", True) :
                legEntries.append( (num, spec["desc"], inDict(spec, "legSpec", legSpec(goptions, inDict(spec, "errorBand", "")))) )
